...
# Nëse jemi në Docker dhe nuk është set-uar ndryshe, përdor emrin e service-it të docker compose: "sqlserver"
...
def load_fact_population(conn, df: pd.DataFrame):
    cursor = conn.cursor()
    count = 0
    ...
    for _, row in df.iterrows():
        ...
        cursor.execute(
            """
            INSERT INTO dwh.fact_population (country_key, time_key, population_value, source_dataset)
            VALUES (?, ?, ?, ?);
            """,
            country_key,
            time_key,
    ...
    conn.commit()
    print(f"U futën {count} rreshta në dwh.fact_population.")
...
def main():
    ...
    expected_cols = ["time", "geo", "measure_value"]
